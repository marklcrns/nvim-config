...
if [ "$(uname)" == "Darwin" ]; then
  ...
  running "Found you use Linux"
	...
	if [ "$(uname -o)" == "Android" ]; then
		...
		if [ -x "$(command -v apk)" ];then sudo apk add bat ripgrep xclip zenity zeal taskwarrior; fi
		if [ -x "$(command -v pkg)" ];then sudo pkg install bat ripgrep xclip zenity zeal taskwarrior; fi
		if [ -x "$(command -v pacman)" ];then sudo pacman -S bat ripgrep xclip zenity zeal taskwarrior; fi
		if [ -x "$(command -v apt)" ]; then sudo apt install bat ripgrep xclip zenity zeal taskwarrior; fi
		if [ -x "$(command -v dnf)" ]; then sudo dnf install bat ripgrep xclip zenity zeal taskwarrior; fi
		if [ -x "$(command -v nix-env)" ]; then sudo nix-env -i bat ripgrep xclip zenity zeal taskwarrior; fi
		if [ -x "$(command -v zypper)" ]; then sudo zypper install bat ripgrep xclip zenity zeal taskwarrior; fi
	fi
fi
